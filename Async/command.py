...
# --- Tempatkan fungsi aexec dan eval_message di sini, sejajar dengan handler ---
...
async def aexec(code: str, client: NewAClient, m: Mess):
    ...
    """
    # Dictionary lokal untuk menyimpan fungsi yang dihasilkan oleh exec
    local_namespace = {}
    
    # Membungkus kode dalam fungsi async
    # 'm' disediakan sebagai alias untuk objek pesan Mess
    # 'client' disediakan untuk interaksi dengan Neonize
    func_code = f"""
...
"""
    ...
    try:
        # Eksekusi pembuatan fungsi async dalam global dan local namespace
        exec(func_code, globals(), local_namespace)
    except SyntaxError as se:
        # Tangkap error sintaks sebelum eksekusi
        raise RuntimeError(f"Syntax Error in code:\n{se.text}\n{' ' * (se.offset - 1)}^\n{type(se).__name__}: {se.msg}")

    # Dapatkan fungsi yang telah dikompilasi
    eval_func = local_namespace.get("__eval_exec")
    if not eval_func:
        raise RuntimeError("Failed to compile code into a function.")

    # Jalankan fungsi async yang telah dibuat dengan parameter client dan m
...
async def eval_message(m: Mess, cmd: str, client: NewAClient):
    ...
    temp_file_name = "neonize_eval_output.txt" # Nama file sementara yang lebih unik
    ...
    try:
        ...
        # Simpan dan alihkan stdout/stderr untuk menangkap output
        ...
        # --- Eksekusi Kode ---
        try:
            # Panggil aexec untuk menjalankan kode
            execution_result = await aexec(cmd, client, m)
        except Exception as e:
            # Tangkap exception yang terjadi selama eksekusi kode
            exception_data = traceback.format_exc()

        # --- Pengumpulan Output ---
        # Pastikan untuk mengembalikan stdout/stderr ke keadaan semula secepatnya
        ...
        if exception_data:
            ...
            # Jika tidak ada output, tampilkan hasil eksekusi (jika ada) atau pesan sukses
            if execution_result is not None:
                 # Coba ubah hasil ke string, tangani objek kompleks jika perlu
                 try:
                     # Untuk objek Neonize seperti JID, MessageInfo dll, __str__ biasanya cukup
                     result_str = str(execution_result)
                 except Exception:
                     # Fallback jika str() gagal
        ...
        # --- Pengiriman Output ---
        max_message_length = 4000 # Batas aman untuk pesan teks WhatsApp
        if len(final_output) > max_message_length:
            ...
            try:
                ...
                await client.send_document(
                    ...
                    quoted=m.message, # Gunakan m.raw untuk objek pesan asli Neonize
                )
                # Edit pesan status
            ...
            # Jika output cukup pendek, kirim langsung dan edit pesan status
            await client.edit_message(m.chat, status_msg_id, Message(conversation=final_output))

    except Exception as outer_error:
        # Tangkap error apa pun yang terjadi di luar proses eval inti (misal, gagal kirim pesan)
        error_trace = traceback.format_exc()
        print(f"[Eval Error - Outer] {outer_error}\n{error_trace}") # Log ke console
        # Kembalikan stdout/stderr jika belum dikembalikan
        ...
        if os.path.exists(temp_file_name):
            try:
                ...
                pass
        # Coba kirim pesan error ke chat
        ...
        try:
            ...
            # Jika gagal mengedit/kirim, abaikan
            pass

# --- Akhir fungsi aexec dan eval_message ---
            ...
            case "edit_message":
                ...
                for i in range(1, len(text) + 1):
                    ...
                    await client.edit_message(message.Info.MessageSource.Chat,
                        ...
                        # --- Bagian case _: dimulai di sini ---
            case _:
                # Cek jika pesan dimulai dengan "=>"
                if budy.startswith("=>"):
                    # Karena pengecekan is_owner sudah dilakukan di awal handler,
                    # kita bisa langsung melanjutkan. Tapi bisa ditambahkan cek lagi jika diperlukan.
                    # if not is_owner:
                    #     await m.reply("❌ Only owner can use eval!")
                    #     return

                    # Ekstrak kode setelah "=>"
                    cmd = budy[2:].strip() # Ambil teks setelah "=>"
                    if not cmd:
                        await m.reply("❌ Please provide code to evaluate. Usage: `!=> print('Hello')`")
                        return

                    # Panggil fungsi eval_message
                    try:
                        await eval_message(m, cmd, client) # Kirim objek m (Mess) dan client
                    except Exception as e:
                        # Tangkap error yang mungkin terjadi sebelum/di luar eval_message itu sendiri
                        error_trace = traceback.format_exc()
                        print(f"[Eval Handler Error] {e}\n{error_trace}") # Log ke console
                        await client.send_message(m.chat, f"💥 *Error in eval handler setup:*\n```python\n{str(e)}\n```\n```traceback\n{error_trace[-500:]}\n```") # Kirim ke chat
                # Tambahkan logika lain untuk command yang tidak dikenali di sini jika diperlukan
                # else:
                #     await m.reply("❌ Command not found!")
            # --- Bagian case _: berakhir di sini ---
    except Exception as e:
        print(f"{e}")
